...
| (a) 除登录外页面≥5 | 导出/演示中可点击进入 ≥5 个页面 | 静态页现有：学生/教师/管理员/统计大屏（4 个）；KesPlus 页面与菜单仍需补齐 | KesPlus 内至少实现 **8+ 页面**（见 §2），并截图目录结构/菜单 |
| (b) 数据统计 + 图表展示 | 至少 2 类图表（柱/折/饼/热力） + 可切换筛选条件 | 已提供 `web/vue-dashboard.html`（Vue3+ECharts）；KesPlus 图表页待补齐 | 统计大屏 + KesPlus 图表页双路径（见 §3），并补充截图与导出 |
| (b) 纯手工设计 Vue 前端页面 | 不依赖模板市场：Vue 页面布局/组件/交互自写 | 已提供 `web/vue-dashboard.html`（手写 Vue3 页面） | 在报告中说明组件/图表/接口联动与导出能力 |
...
| (d) 报告完整详实 | 报告含：数据库设计、功能介绍、运行截图、测试用例、部署步骤 | 已有占位稿 `report/高校选课管理系统设计报告.md`；仍需补齐 KesPlus 截图与最终 PDF/Word | 按 `项目报告模板.txt` 输出正式报告 + 截图清单（见 §6） |
