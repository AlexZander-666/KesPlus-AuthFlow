...
VALUES ('C010', 'Conflict Course', 'Elective', 2.0, 32, 1, 1, '2024-2025-1', 1, 1, 'for conflict test', '1', 1, 1, 2, 'Z101');
INSERT INTO TB_COURSE_TIME (COURSE_ID, TERM, DAY_OF_WEEK, START_SLOT, END_SLOT, LOCATION)
SELECT COURSE_ID, TERM, DAY_OF_WEEK, START_SLOT, END_SLOT, LOCATION FROM TB_COURSE WHERE COURSE_NO='C010';
SELECT 'C1_conflict_check' AS step, * FROM FN_CHECK_TIME_CONFLICT(1, (SELECT COURSE_ID FROM TB_COURSE WHERE COURSE_NO='C010'), '2024-2025-1');

-- 将课程2设为满员，加入候补并处理
UPDATE TB_COURSE SET CAPACITY = 1, SELECTED_NUM = 1 WHERE COURSE_NO='C002';
SELECT 'W1_join_waitlist' AS step, * FROM FN_JOIN_WAITLIST(3, (SELECT COURSE_ID FROM TB_COURSE WHERE COURSE_NO='C002'), '2024-2025-1');
SELECT 'W2_waitlist_by_student' AS step, * FROM FN_WAITLIST_BY_STUDENT(3, '2024-2025-1');
UPDATE TB_COURSE SET SELECTED_NUM = 0 WHERE COURSE_NO='C002'; -- 腾出名额
SELECT 'W3_process' AS step, * FROM FN_PROCESS_WAITLIST((SELECT COURSE_ID FROM TB_COURSE WHERE COURSE_NO='C002'), '2024-2025-1');
SELECT 'W4_verify_enrolled' AS step, STATUS FROM TB_STUDENT_COURSE WHERE STU_ID=3 AND COURSE_ID=(SELECT COURSE_ID FROM TB_COURSE WHERE COURSE_NO='C002');
SELECT 'W5_waitlist_status' AS step, STATUS, MESSAGE FROM TB_WAITLIST WHERE STU_ID=3 AND COURSE_ID=(SELECT COURSE_ID FROM TB_COURSE WHERE COURSE_NO='C002');
