...
## 6. 测试记录（本地 PostgreSQL/Kingbase，参数参考 `server/.env.example`）
执行顺序：
```powershell
$env:PGPASSWORD='<你的数据库密码>'  # 不要写入仓库/报告公开版本
...
psql -h <DB_HOST> -p <DB_PORT> -U <DB_USER> -d <DB_NAME> -f tests/db_trigger_procedure_tests.sql
```
摘要结果：
...
- 数据库备份：如需提交逻辑备份，可使用 `backups/course_selection_db.sql`（或替换为 KStudio/Kingbase 实际导出的 SQL）。
